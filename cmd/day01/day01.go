...
In this example, the first frequency reached twice is 2. Note that your device might need to repeat its list of frequency changes
many times before a duplicate frequency is found, and that duplicates might be found while in the middle of processing the list.
...
func run() error {
	...
	firstRepeat, err := firstRepeatFrequency(input)
	if err != nil {
		return err
	}
...
// firstRepeatFrequency returns the first frequency reached twice. Repeatedly iterating
// over the input if necessary.
func firstRepeatFrequency(input string) (int, error) {
	input = strings.TrimSpace(input)
	...
	frequency := 0
	seen := set.New[int]()
	seen.Insert(frequency) // Include the starting frequency of 0

	for {
		for _, line := range lines {
			change, err := parseFrequencyChange(line)
			if err != nil {
				return 0, err
			}

			frequency += change
			if newlyInserted := seen.Insert(frequency); !newlyInserted {
				// It already existed meaning this is the first one to be
				// seen twice
				return frequency, nil
			}
		}
